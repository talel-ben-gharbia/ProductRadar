...
export async function GET(request: NextRequest) {
  ...
  const targetUrl = `${BACKEND_URL}/admin/api/scraping/manual/default-link${request.nextUrl.search}`
  ...
  try {
    ...
    if (!response.ok) {
      const error = (data as { error?: string }).error || "Failed to fetch category link."
