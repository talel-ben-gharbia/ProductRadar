...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${BACKEND_URL}/admin/api/scraping/manual/trigger`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      const error = (data as { error?: string }).error || "Failed to trigger manual scrape."
      return NextResponse.json({ ...(data as object), error }, { status: response.status })
    }
