...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    const response = await fetch(`${BACKEND_URL}/admin/api/quality/product-listings/${encodeURIComponent(id)}/split`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      const error = (data as { error?: string }).error || "Split operation failed."
